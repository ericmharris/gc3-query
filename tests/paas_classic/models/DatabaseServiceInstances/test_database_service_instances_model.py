...
#     service_cfg, idm_cfg, paas_service = setup_gc30003_dump
...
def setup_gc30003_dbcs_service_instances():
    ...
    service_cfg = gc3_cfg.paas_classic.services.get(paas_type)[service]
    idm_cfg = gc3_cfg.idm.domains[idm_domain]
    mongodb_connection: MongoClient = storage_adapter_init(mongodb_config=gc3_cfg.paas_classic.mongodb.as_dict())
    paas_service: DatabaseServiceInstances = DatabaseServiceInstances(service_cfg=service_cfg, idm_cfg=idm_cfg)
    assert service==service_cfg.name
    assert idm_domain==idm_cfg.name
    assert gc3_cfg.user.cloud_username == 'eric.harris@oracle.com'
    yield service_cfg, idm_cfg, paas_service, mongodb_connection


def test_dump(setup_gc30003_dbcs_service_instances):
    service_cfg, idm_cfg, paas_service, mongodb_connection = setup_gc30003_dbcs_service_instances
    service_response = paas_service.dump()
    assert service_response.result
    result = service_response.result


def test_save_all(setup_gc30003_dbcs_service_instances):
    service_cfg, idm_cfg, paas_service, mongodb_connection = setup_gc30003_dbcs_service_instances
    ...
    for result in results:
        ...
        sec_application_model = DatabaseServiceInstancesModel(**result_dict)
