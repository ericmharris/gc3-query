...
"""
gc3-query.test_iaas_sec_ip_lists    [9/8/2018 1:35 PM]
~~~~~~~~~~~~~~~~
...
def test_init(setup_gc30003):
    ...
    sec_ip_lists = SecIPLists(service_cfg=service_cfg, idm_cfg=idm_cfg, http_client=http_client)
    assert sec_ip_lists.http_client.skip_authentication == False
    assert sec_ip_lists.http_client.idm_domain_name == idm_cfg.name
    assert sec_ip_lists.http_client.auth_cookie_header is not None
    assert 'nimbula' in sec_ip_lists.http_client.auth_cookie_header['Cookie']


def test_list_all_sec_ip_lists(setup_gc30003):
    service_cfg, idm_cfg, http_client = setup_gc30003
    sec_ip_lists = SecIPLists(service_cfg=service_cfg, idm_cfg=idm_cfg, http_client=http_client)
    assert 'nimbula' in sec_ip_lists.http_client.auth_cookie_header['Cookie']
    all_sec_ip_lists = sec_ip_lists.get_all_sec_ip_lists()
    assert all_sec_ip_lists
