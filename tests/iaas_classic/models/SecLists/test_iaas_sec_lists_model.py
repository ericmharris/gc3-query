...
def test_list_sec_lists_model_save_from_url():
    ...
    sec_list_model = SecRuleModel(**result_json)
...
def test_save_one(setup_gc30003_model):
    ...
    sec_list_model = SecRuleModel(**first_result_dict)
    saved = sec_list_model.save()
    assert saved
...
def test_save_all(setup_gc30003_model):
    ...
    for result in results:
        ...
        sec_list_model = SecRuleModel(**result_dict)
        saved = sec_list_model.save()
...
def test_insert_all(setup_gc30003_model):
    ...
    assert service_response.result
    results = service_response.result.result
    sec_lists = [SecRuleModel(**result._as_dict()) for result in results]
    _ = SecRuleModel.objects().insert(sec_lists)
    assert sec_lists
