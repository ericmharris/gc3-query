...
def test_list_sec_lists_model_from_url():
    ...
    # http_future = sec_lists.bravado_service_operations.listSecList(container=container)
...
def test_list_sec_lists_model_save_from_url():
    ...
    # http_future = sec_lists.bravado_service_operations.listSecList(container=container, _request_options={"headers": {"Accept": ','.join(callable_operation.operation.produces)}})
    http_future = sec_lists.service_operations.list_sec_list(container=container)
...
def test_query_objects(setup_gc30003_model_query):
    ...
    # http_client: IaaSRequestsHTTPClient = IaaSRequestsHTTPClient(idm_cfg=idm_cfg)
    objects: QuerySet = SecListModel.objects()
    assert objects
