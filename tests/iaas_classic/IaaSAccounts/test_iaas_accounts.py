...
def test_init():
    ...
    accounts = Accounts(service_cfg=service_cfg, idm_cfg=idm_cfg, from_url=True)
    assert accounts.name==service
#
#
...
#     assert idm_domain==idm_cfg.name
#     assert gc3_config.user.cloud_username == 'eric.harris@oracle.com'
#     accounts = Accounts(service_cfg=service_cfg, idm_cfg=idm_cfg)
#     # container=accounts.idm_container_name.replace('/', '')
#     container=accounts.idm_root_container_name
#     old_container=accounts.idm_container_name
#     # http_future = accounts.bravado_service_operations.listInstance(container=accounts.idm_user_container_name)
#     # http_future = accounts.bravado_service_operations.listInstance(container=accounts.idm_container_name)
#     # http_future = accounts.bravado_service_operations.listSecRule(container=container)
#     http_future = accounts.service_operations.list_sec_rule(container=container)
#     # http_future = accounts.service_operations.discover_root_instance()
...
#
#
# def test_get_all_accounts_data_types_correct():
#     service = 'Accounts'
#     idm_domain = 'gc30003'
#     gc3_config = GC3Config(atoml_config_dir=config_dir)
#     service_cfg = gc3_config.iaas_classic.services[service]
...
#     assert gc3_config.user.cloud_username == 'eric.harris@oracle.com'
#     accounts = Accounts(service_cfg=service_cfg, idm_cfg=idm_cfg)
#     result_json = accounts.get_all_accounts()
#     assert len(result_json['result']) > 0
#     assert 'src_list' in result_json['result'][0]
#     assert isinstance(result_json['result'][0]['dst_is_ip'], bool)
#
...
#     service = 'Accounts'
#     idm_domain = 'gc30003'
#     spec_file_name = 'Accounts_string_type.json'
...
#     assert idm_domain==idm_cfg.name
#     assert gc3_config.user.cloud_username == 'eric.harris@oracle.com'
#     accounts = Accounts(service_cfg=service_cfg, idm_cfg=idm_cfg, spec_dict=spec_dict, swagger_spec=swagger_spec)
#     assert 'boolean_string' in [f.format for f in accounts.bravado_config['formats']]
#     result_json = accounts.get_all_accounts()
#     assert len(result_json['result']) > 0
#     assert 'src_list' in result_json['result'][0]
