...
class ReadConfigFile(ConfigParser.RawConfigParser):
    """
    based on ConfigParser from the standard library, modified to parse atoml
...
class Utils(object):
    # a general class to get atoml values and general utility apis.
    ...
    def __init__(self):
        ...
        # arguments for setup. atoml payload
        self._config_payload_params = ['-c', '--atoml-payload']
    ...
    def getValueFromConfigFile(self, value):
        # reads the value from the atoml value for specified key.
    ...
    def writeValueToConfigFile(self, key, value):
        # write the value to the atoml file. either adds a value
    ...
    def getAuthToken(self):
        ...
        if token is None:
            # raise atoml error when there is no token
            raise OpaasConfigError
        return token
    ...
    def remove_config_dirs(self, dir_name):
        # remove all the atoml dirs while doing psm undo setup
        try:
