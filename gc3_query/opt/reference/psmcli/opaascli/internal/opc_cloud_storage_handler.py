    ...
    def _upload_to_storage_cloud(self, url, source, usr=None, pwd=None, auth_token=None, delete_header=False):
        ...
        try:
            with open(source, 'rb') as fileObj:        
                ...
                response = self.do_request(method='PUT', request_url=url, headers=headers, data=mmapped_data, stream=True) #files={'archive' : (os.path.basename(source), fileObj, 'application/x-gzip')})
                mmapped_data.close()
